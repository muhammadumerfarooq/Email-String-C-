...
#include<cstring>
using namespace std;
int case1(char subject[100])  // case 1 check where subject string is check with the Capital alphabets wordss 
{
	int i = 0;
	int totalwords = 0;
	int capital = 0;
...
{
	int i = 0;
	while (subject[i] != '\0')  // in this code 
	{

		int count = 0;
		while (subject[i] != ' '&& subject[i] != '\0')  //comparing consanants with ascii values 
		{
			int val = subject[i];
...
int case3(char subject[100])
{
	...
	while (subject[i] != '\0')
	{
		while (subject[i] != ' ' && subject[i] != '\0') // comparing excalamatory sign with character array
		{
			if (subject[i] == '!')
			{
...
int case4(char body[100])
{
	...
	while (body[i] != '\0')
	{
		bool vari = true;
		bool check = false;
		while (body[i] != ' ' && body[i] != '\0') // checking if words are in capital alphabets by comapring with the ascii values
		{
...
int case5(char body[100])
{
	...
	while (body[i] != '\0')
	{
		...
		while (body[i] != '\0' && body[i] != ' ')
		{
			...
			i++;   // comapring all the words in string with these words 
...
string  getFirstWord(char words[100])  // return first word in string if space apperas r  '\0' it stops 
{
	int i = 0;
	while (words[i] != '\0')
	{
...
string getLastWord(char mesage[100])  // return last words to check it it compares the chacter with '\0'
{
...
string extractWord(char(&mesage)[100])  // pass the refernce in parameter to exclude the word whihc is first word 
{
...
void test()  // test case to test all the string functions
{
...
bool isGibberishWord(char mesage[100]) // check consectuve exlamatory mrks in string 
{
	//if (val != 65 && val != 97 && val != 69 && val != 101 && val != 73 && val != 105 && val != 79 && val != 111 && val != 85 && val != 117 && ((val>65 && val <= 90) || (val>97 && val <= 122)))
	//{
